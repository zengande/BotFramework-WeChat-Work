...
using Microsoft.Bot.Builder.Adapters.WeChat.Work.Storage;
...
namespace Microsoft.Bot.Builder.Adapters.WeChat.Work
{
    ...
    internal class WeChatWorkClient : IDisposable
    {
        ...
        {
            var token = await _tokenStorage.GetAsync(_settings.CorpId).ConfigureAwait(false);
            ...
            if (token == null || forceRefresh)
            {
                try
                {
                    ...
                    token = await _tokenStorage.GetAsync(_settings.CorpId).ConfigureAwait(false);
                    if (token == null || forceRefresh)
                    {
